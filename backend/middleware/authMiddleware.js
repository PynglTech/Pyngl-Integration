...
const protect = async (req, res, next) => {
  ...
  if (!token) {
    return res.status(401).json({ error: "Login required" }); // Only if no login at all
  }
  ...
  try {
    ...
    // DO NOT force logout if phone number is missing
    // allow request to go ahead so frontend can ask user to enter phone manually
    req.hasPhone = !!user.phoneNumber;
