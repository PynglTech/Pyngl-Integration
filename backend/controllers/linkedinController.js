...
const LINKEDIN_CLIENT_SECRET = process.env.LINKEDIN_CLIENT_SECRET;
// IMPORTANT: This URI must match what's in your LinkedIn Dev App settings
const LINKEDIN_REDIRECT_URI = 'https://localhost:5000/api/linkedin/auth/callback';
...
function generateTrackableLink(pollId) {
    const params = new URLSearchParams({ utm_source: 'linkedin', utm_medium: 'native_poll' });
    return `https://localhost:5173/poll/${pollId}?${params.toString()}`;
}
...
export const handleLinkedInCallback = async (req, res) => {
  try {
    ...
    if (!code) {
      return res.redirect('https://localhost:5173/share-linkedin?auth_linkedin=failed');
    }
    ...
    const redirectUrl = pollId
      ? `https://localhost:5173/share-linkedin?auth_linkedin=success&pollId=${pollId}`
      : `https://localhost:5173/share-linkedin?auth_linkedin=success`;

    res.redirect(redirectUrl);
  } catch (error) {
    console.error("LinkedIn Auth Error:", error.response?.data || error.message);
    res.redirect("https://localhost:5173/share-linkedin?auth_linkedin=failed");
  }
};
