...
const FaqItem = ({ question, answer }) => {
    const [isOpen, setIsOpen] = useState(false);
    return (
        <div className="border border-gray-200 dark:border-gray-700 rounded-2xl bg-white dark:bg-gray-800">
            <button
                onClick={() => setIsOpen(!isOpen)}
                className="w-full flex justify-between items-center p-4 text-left"
            >
                <span className="font-semibold text-gray-800 dark:text-gray-100">{question}</span>
                {isOpen ? <Minus size={20} className="text-pink-500" /> : <Plus size={20} className="text-gray-500 dark:text-gray-400" />}
            </button>
            {isOpen && (
...
const ContactItem = ({ icon, title, link }) => {
    return (
        <a
            ...
            className="w-full flex items-center gap-4 p-4 border border-gray-200 dark:border-gray-700 rounded-2xl bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"
        >
            {icon}
...
export default function HelpCenter() {
    ...
    const contactLinks = [
        { icon: <Headset className="text-blue-500"/>, title: 'Customer Services', link: 'mailto:support@pyngl.com' },
        { icon: <MessageSquare className="text-green-500"/>, title: 'WhatsApp', link: 'https://wa.me/your-whatsapp-number' },
        { icon: <Globe className="text-gray-500 dark:text-gray-400"/>, title: 'Website', link: 'https://pyngl.com' },
    ];
                ...
                <div className="space-y-4">
                    {activeTab === 'FAQs'
                        ...
                        : contactLinks.map((item, index) => (
                              <ContactItem key={index} icon={item.icon} title={item.title} link={item.link} />
                          ))}
                </div>
            </div>
        </div>
