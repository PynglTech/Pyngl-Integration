...
// Animated SVG icon for poll notifications
...
const NotificationItem = ({ notification, onWelcomeClick }) => {
    ...
    const handleClick = async () => {
        ...
        if (message.includes('welcome')) {
            if (!notification.read) await handleNotificationClick(notification);
            onWelcomeClick();
        } else {
    ...
    return (
        <div ref={itemRef} className="relative">
            ...
            <animated.div
                ...
                onClick={handleClick}
                className={`
                    relative z-10 flex items-start gap-4 p-4 rounded-2xl border
...
export default function NotificationsPage() {
    ...
    useEffect(() => {
        ...
        if (unreadWelcome) setShowWelcome(true);
    }, [notifications]);
