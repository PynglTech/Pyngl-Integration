...
export default function PreviewTextPoll() {
    ...
    const handleCreatePoll = async () => {
        ...
        try {
            ...
            console.error(error);
            toast.error(error.response?.data?.error || "Error saving poll.");
        } finally {
            setIsLoading(false);
        }
    };
    ...
    // Auto-create poll on component mount
    useEffect(() => {
        if (!createdPoll) {
            handleCreatePoll();
        }
                        ...
                        <div ref={pollRef} className="p-6 bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700">
                            ...
                            <div className="space-y-3">
                                {state.options.map((opt, i) => (
                                    ...
                                    <div key={i} className="w-full border rounded-full px-4 py-3 text-left text-gray-700 dark:text-gray-200 border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-700">
                                        {opt}
                                    </div>
                                ))}
                            </div>
                        ...
                        <div ref={pollRef} className="p-4 bg-white dark:bg-gray-900 rounded-xl shadow-md">
                            <div className="rounded-xl border border-gray-200 dark:border-gray-700 p-4">
                                ...
                                <div className="space-y-3">
                                    {state.options.map((opt, i) => (
                                        // THIS IS THE FIX: text-left is added for clarity
                                        <div key={i} className="w-full border rounded-full px-4 py-2 text-left text-gray-700 dark:text-gray-100 border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-700">
                                            {opt}
                                        </div>
                                    ))}
                                </div>
