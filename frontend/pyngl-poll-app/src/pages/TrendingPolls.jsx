...
//         <circle cx="11" cy="11" r="8" stroke="url(#search-gradient)" strokeWidth="3" />
//         <line x1="16.65" y1="16.65" x2="21" y2="21" stroke="url(#search-gradient)" strokeWidth="3" strokeLinecap="round" />
//     </svg>
// );
...
useEffect(() => {
        ...
        try {
            ...
            const mappedPolls = dbPolls
                .map((poll) => ({
                    ...
                    // The 'category' field will be added in the next step
                }))
                .filter((poll) => !votedPolls.includes(poll.id));

            // --- THIS IS THE FIX ---
            // 1. Run the categorization algorithm
            const categorizedPolls = categorizePolls(mappedPolls);
            
            // 2. Save the ENTIRE categorized list to your state
            setAllPolls(categorizedPolls);
            setVisiblePolls(categorizedPolls); // Also update the visible polls

            // 3. Create the tabs from this categorized list
            const categorySet = new Set(
                categorizedPolls.map((p) => p.category).filter(Boolean)
            );
            setTabs(["All", ...Array.from(categorySet)]);
    ...
    fetchPolls();
}, []); // Dependency array can be empty as it only runs once
  // --- Filtering + Search ---
  const filteredPolls =
    selectedTab === "All"
      ? visiblePolls
