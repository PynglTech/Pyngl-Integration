                    ...
                    {(poll.options || []).map((option, index) => {
                        ...
                        return (
                            <div key={option._id || index} className="w-full bg-gray-100 dark:bg-gray-700 rounded-lg h-10 flex items-center relative overflow-hidden">
                                ...
                                />
                                <div className="absolute inset-0 px-3 flex justify-between items-center">
                                    <span className="font-medium text-gray-800 dark:text-gray-200 mix-blend-hard-light">{option.text}</span>
                                    <span className="font-semibold text-gray-700 dark:text-gray-300 mix-blend-hard-light">{percent}%</span>
                                </div>
                            </div>
                        );
                    })}
...
export default function Analytics() {
    ...
    const finalPolls = polls.filter((poll) => {
        ...
        const matchPlatform =
            platformFilter === "All" || (poll.sharedPlatforms || []).includes(platformFilter.toLowerCase());
        ...
        <div className="bg-gray-50 dark:bg-gray-900 min-h-screen text-gray-900 dark:text-gray-200">
            {isDesktop ? (
                    ...
                    <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full">
                        ...
                        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
                            <div className="flex bg-gray-100 dark:bg-gray-800 rounded-full p-1 space-x-1">
                                {["My Pings", "Participated Pings"].map((tab) => (
                                    <button key={tab} onClick={() => setActiveTab(tab)} className={`flex-1 px-6 py-2 text-sm font-medium rounded-full transition-all ${activeTab === tab ? "bg-white dark:bg-gray-700 text-gray-800 dark:text-white shadow" : "text-gray-500 dark:text-gray-400 hover:bg-white/50 dark:hover:bg-gray-700/50"}`}>{tab}</button>
                                ))}
                            </div>
                            <div className="flex items-center gap-4">
                                <div className="relative flex-1 min-w-[300px]">
                                    <input type="text" placeholder="Search by keyword..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-10 pr-4 py-3 bg-white dark:bg-gray-800 rounded-full border border-gray-200 dark:border-gray-700 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500" />
                                    <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" />
                                </div>
                                <button onClick={() => setShowFilter(true)} className="h-12 w-12 flex items-center justify-center rounded-xl border bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 flex-shrink-0 hover:bg-gray-50 dark:hover:bg-gray-700"><Filter className="w-5 h-5 text-gray-600 dark:text-gray-300" /></button>
                            </div>
                        </div>

                        {/* Polls Grid */}
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                            {loading ? <InlineLoader text="Fetching your pings..." /> : finalPolls.length > 0 ? (finalPolls.map((poll) => <AnalyticsPollCard key={poll.id} poll={poll} onSelect={setSelectedPoll} />)) : (<div className="text-center text-gray-500 mt-10 col-span-full"><p>No polls found.</p></div>)}
                        </div>
                    </main>
                </>
            ) : (
                ...
                <div className="mx-auto max-w-7xl">
                    <div className="sticky top-0 z-40 bg-white/80 dark:bg-gray-800/80 backdrop-blur-md border-b border-gray-100 dark:border-gray-700">
                        <div className="flex items-center justify-between p-4"><button onClick={() => navigate(-1)}><ArrowLeft className="w-6 h-6" /></button><h1 className="text-lg font-semibold">Analytics</h1><button onClick={() => navigate('/notifications')}><Bell className="w-6 h-6" /></button></div>
                    </div>
                    <div className="flex flex-wrap items-center justify-between px-4 mt-4 gap-2">
                        <div className="flex bg-gray-100 dark:bg-gray-800 rounded-full p-1 space-x-1 flex-1 min-w-[280px]">{["My Pings", "Participated Pings"].map((tab) => ( <button key={tab} onClick={() => setActiveTab(tab)} className={`flex-1 px-4 py-2 text-sm font-medium rounded-full transition-all ${ activeTab === tab ? "bg-gradient-to-r from-pink-500 to-purple-500 text-white shadow" : "text-gray-600 dark:text-gray-300 hover:bg-white/50 dark:hover:bg-gray-700/50" }`}>{tab}</button>))}</div>
                        <button onClick={() => setShowFilter(true)} className="w-10 h-10 flex items-center justify-center rounded-xl border bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700"><Filter className="w-5 h-5 text-gray-600 dark:text-gray-300" /></button>
                    </div>
                    <div className="px-4 mt-4">
                        <input type="text" placeholder="Search by keyword..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-4 pr-10 py-3 bg-white dark:bg-gray-800 rounded-full border border-gray-200 dark:border-gray-700 text-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-purple-500" />
                    </div>
                    <div className="mt-3 px-4 grid grid-cols-1 sm:grid-cols-2 gap-4 pb-24 pt-4">
                        {loading ? <InlineLoader text="Fetching your pings..." /> : finalPolls.length > 0 ? (finalPolls.map((poll) => ( <AnalyticsPollCard key={poll.id} poll={poll} onSelect={setSelectedPoll} />))) : (<div className="text-center text-gray-500 mt-10 col-span-1 sm:col-span-2"><p>No polls found for this category.</p></div>)}
                    </div>
                </div>
            )}

            {/* Modals are shared between both layouts */}
            {showFilter && <FilterDrawer show={showFilter} setShow={setShowFilter} statusFilter={statusFilter} setStatusFilter={setStatusFilter} platformFilter={platformFilter} setPlatformFilter={setPlatformFilter} />}
            {selectedPoll && <PollDetailPopup poll={selectedPoll} onClose={() => setSelectedPoll(null)} onSeeAnalytics={() => { navigate(`/analytics/${selectedPoll.id}`); }} />}
        </div>
    );
}
