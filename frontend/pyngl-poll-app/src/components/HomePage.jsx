...
// (Your inline SVG icons TfiTextIcon and FaRegImageIcon remain the same)
...
const PollCard = ({ poll }) => {
    ...
    return (
        <Link to={`/poll/${poll._id}`} className="block rounded-xl border border-gray-200 p-4 mb-4 bg-white hover:bg-gray-50 transition-colors">
            ...
            <div className="space-y-2">
                {poll.options.slice(0, 2).map((option) => (
                    <div key={option._id} className="w-full border rounded-full px-4 py-2 text-left text-gray-700 bg-gray-50 truncate">
                        {option.text}
                    </div>
                ))}
            </div>
...
const HomePage = () => {
     ...
     useEffect(() => {
        const fetchLivePolls = async () => {
            try {
                ...
                // 2. Switched from 'fetch' to the central 'apiClient'
                const response = await apiClient.get('/api/polls/live');
                const polls = response.data; // With Axios, the data is in response.data
                ...
                // The global interceptor in apiClient will handle 401 errors automatically
            } finally {
    ...
    // --- NEW: Effect to sort polls based on the active filter ---
    useEffect(() => {
    ...
    return (
        <div className="home-container">
            {/* (Upgrade Card and Poll Options Sections remain the same) */}
            ...
            {/* --- UPDATED: Live Polls Section with Filters --- */}
            <div className="mt-6">
                <div className="flex justify-between items-center mb-4">
                    <div className="flex items-center gap-2">
                        <h3 className="text-xl font-bold text-gray-800">Live polls</h3>
                ...
                <div>
                    {isLoadingPolls ? (
                        <p className="text-center text-gray-500 py-8">Loading polls...</p>
                    ) : filteredPolls.length > 0 ? (
                        filteredPolls.map((poll) => (
                            <PollCard key={poll._id} poll={poll} />
                        ))
                    ) : (
                        <p className="text-center text-gray-500 py-8">No live polls at the moment. Why not create one?</p>
                    )}
                </div>
            </div>
        </div>
