...
const shareLinks = {
    ...
    imessages: (previewUrl, text) => `sms:?body=${encodeURIComponent(text + "\n" + previewUrl)}`,
...
const ShareButton = ({ platform, onSelect, isSelected, completed }) => (
    <div className="flex flex-col items-center relative">
        ...
        <span className="mt-2 text-xs font-medium capitalize text-gray-600 dark:text-gray-300">
            {platform === "sms" ? "SMS" : platform === "copy" ? "Copy Link" : platform === "imessages" ? "iMessage" : platform}
        </span>
    </div>
);
...
export default function ShareSheet({
    ...
    // Added 'imessages' to the platforms list so it appears in the grid
    const platforms = ['instagram', 'whatsapp', 'imessages', 'gmail', 'linkedin', 'twitter', 'facebook', 'sms', 'telegram', 'copy', 'youtube'];
    ...
    const handleConfirmShare = async (hostedPreviewImage) => {
        ...
        try {
            if (platform === 'linkedin') {
                ...
                if (data.isAuthenticated) {
                    ...
                    toast((t) => (
                        <div className="flex flex-col items-center text-center">
                            <p className="mb-3 text-gray-800 font-medium">Connect your LinkedIn account</p>
                            <button
                                onClick={() => {
                                    toast.dismiss(t.id);
                ...
                try {
                    const { data } = await apiClient.post("/api/telegram/sharePoll", { pollId: poll._id }, { withCredentials: true });
                    if (data.linked) {
                        ...
                        toast((t) => (
                            <div className="flex flex-col items-center text-center">
                                <p className="mb-3 text-gray-800 font-medium"><b>Link Your Telegram Account</b><br />Please tap "Open" and then hit <strong>"Start"</strong> in Telegram to connect.</p>
                                <a href={data.url} target="_blank" rel="noopener noreferrer" className="px-5 py-2 rounded-full bg-sky-600 text-white font-semibold hover:bg-sky-700 transition" onClick={() => toast.dismiss(t.id)}>
                                    Open @PynglPollsBot
                                </a>
                            </div>
                        ), { duration: 10000 });
            ...
            // --- PlatformPreview handles iMessage logic internally ---
            else if (platform === "imessages") {
                // No special logic needed here because PlatformPreview.jsx
                // handles the actual sharing/linking for iMessage.
                // We just fall through to the analytics logging below.
            }
            else if (platform !== 'instagram') {
                // For generic platforms (Twitter, Facebook, SMS)
                const shareUrl = shareLinks[platform](previewUrl, poll.question);
                if (shareUrl) {
